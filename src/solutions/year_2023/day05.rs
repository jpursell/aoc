...
impl SeedRange {
    fn make_vec(map: &[&str]) -> Vec<SeedRange> {
        assert_eq!(map.len(), 1);
        let line = map[0];
...
impl RangeMap {
    ...
    fn contains_key(&self, key: &u64) -> bool {
        ...
        key - self.source_start < self.range
    }
    ...
    fn contains_dest(&self, dest: &u64) -> bool {
        ...
        dest - self.dest_start < self.range
    }
...
impl Plan {
    fn new(input: &str) -> Plan {
        ...
        for map in maps_vec {
            maps.insert(map.input.clone(), map);
        }
        Plan { seeds, maps }
    }

    fn map_seeds(&self, dest: &str) -> Vec<u64> {
        let mut out = Vec::new();
...
fn is_seeds(map: &[&str]) -> bool {
    map.len() == 1
}

fn day_5a(input: &str) -> u64 {
    *Plan::new(input).map_seeds("location").iter().min().unwrap()
}
...
impl AocSolution for Day05 {
    fn part1(&self, input: &str) -> String {
        day_5a(input).to_string()
    }

    fn part2(&self, input: &str) -> String {
        day_5b(input).to_string()
    }
}
...
56 93 4"#;
    ...
    #[test]
    fn test_part1_full() {
        let input = crate::get_input_for_day(2023, 5).expect("Failed to get input");
        assert_eq!(Day05.part1(&input), "806029445");
    }
    ...
    fn test_part2_full() {
        let input = crate::get_input_for_day(2023, 5).expect("Failed to get input");
        assert_eq!(Day05.part2(&input), "59370572");
    }
}
