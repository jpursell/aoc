...
impl Puzzle {
    fn process_p1(&self) -> usize {
        ...
        while let Some((new_position, new_direction)) = self.update_p1(position, direction) {
    ...
    fn check_if_block_makes_loop(&mut self, block_location: [usize; 2]) -> bool {
        ...
        while let Some((new_position, new_direction)) = self.update_p2(position, direction) {
            position = new_position;
            direction = new_direction;
...
......#...";
    ...
    fn test_part1_example() {
        assert_eq!(Day06.part1(EXAMPLE), "41");
    }

    #[test]
    fn test_part1_full() {
        let input = crate::get_input_for_day(2024, 6).expect("Failed to get input");
        assert_eq!(Day06.part1(&input), "5534");
    }

    #[test]
    fn test_part2_example() {
        assert_eq!(Day06.part2(EXAMPLE), "6");
    }

    #[test]
    fn test_part2_full() {
        let input = crate::get_input_for_day(2024, 6).expect("Failed to get input");
        assert_eq!(Day06.part2(&input), "2262");
    }
}
